...
export default function ProfilePage() {
  ...
  const load = useCallback(
    () =>
      adminFetch<Detail>("/api/admin/profile")
        .then((value) => {
          setUser(value);
          setName(value.name);
  ...
  async function profile(e: FormEvent) {
    ...
    try {
      ...
      setMessage(x instanceof Error ? x.message : "Profil gagal diperbarui.");
    }
  }
  async function password(e: FormEvent) {
    ...
    try {
      ...
      setMessage(x instanceof Error ? x.message : "Gagal mengubah password.");
    }
  }
  return (
    <>
      ...
      <div className={styles.grid}>
        <form className={styles.userForm} onSubmit={profile}>
          ...
          <p>
            <b>Bergabung:</b> {user ? new Date(user.created_at).toLocaleString("id-ID") : "—"}
          </p>
          <p>
            <b>Login terakhir:</b>{" "}
        ...
        <form className={styles.userForm} onSubmit={password}>
          ...
          {user?.activities.map((item) => (
            <div className={styles.systemRow} key={item.id}>
              ...
              <div>
                <b>{item.action}</b>
                <small>
                  {item.module ?? "sistem"} · {item.detail ?? "—"}
                </small>
              </div>
