...
export default function SettingsPage() {
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      setMessage("Pengaturan berhasil disimpan dan dicatat dalam log.");
    } catch (x) {
      setMessage(x instanceof Error ? x.message : "Pengaturan gagal disimpan.");
    } finally {
      setSaving(false);
    }
  }
  return (
    <>
      ...
      <form className={styles.settingsGrid} onSubmit={save}>
        {Object.entries(values).map(([group, items]) => (
          <fieldset className={styles.panel} key={group}>
            <legend>{group.replaceAll("_", " ")}</legend>
            {Object.entries(items).map(([key, value]) => (
              <label key={key}>
                {key.replaceAll("_", " ")}{" "}
                {value === "true" || value === "false" ? (
                  <select value={value} onChange={(e) => set(group, key, e.target.value)}>
                    <option value="true">Aktif</option>
                    <option value="false">Nonaktif</option>
                  </select>
                ) : (
                  <input value={value} onChange={(e) => set(group, key, e.target.value)} />
                )}
              </label>
            ))}
          </fieldset>
