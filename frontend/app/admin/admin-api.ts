...
export type Candidate = {
  ...
  category: string;
  source: string;
  data_type: "primer" | "sekunder" | "sintetis";
  validation_status: "pending" | "approved" | "rejected" | "verified";
...
export function authHeaders(json = false): HeadersInit {
  const token =
    localStorage.getItem("nusaguard_token") ?? sessionStorage.getItem("nusaguard_token") ?? "";
  return json
...
export async function adminLogout() {
  try {
    ...
    sessionStorage.removeItem("nusaguard_token");
  }
}
export async function adminDownload(path: string, filename: string) {
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => ({}));
    throw new Error(body.detail ?? "Unduhan gagal.");
  }
