...
export default function Profile({
  ...
  user: { name: string; email: string; last_login_at?: string; created_at?: string };
  ...
  async function profile(e: FormEvent) {
    e.preventDefault();
    await userFetch("/api/user/profile", { method: "PATCH", body: JSON.stringify({ name }) });
    onUpdate(name);
    setMessage("Profil diperbarui.");
  }
  async function password(e: FormEvent) {
    e.preventDefault();
    await userFetch("/api/user/password", {
      method: "POST",
  ...
  return (
    <section>
      ...
      <div className={styles.profileGrid}>
        <form className={styles.settings} onSubmit={profile}>
        ...
        <form className={styles.settings} onSubmit={password}>
