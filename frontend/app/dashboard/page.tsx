...
export default function Dashboard(){
  ...
  return <div className={`${styles.shell} ${menuOpen?styles.menuOpen:""}`}>
    ...
    <main>
      <header><button className={styles.menuButton} onClick={()=>setMenuOpen(true)} aria-label="Buka menu">☰</button><div><p>PUSAT KEAMANAN / {view.toUpperCase()}</p><h1>{view==="home"?`Halo, ${user.name.split(" ")[0]}.`:view==="analyze"?"Analisis pesan":"Hak akses akun"}</h1></div><span>● TERLINDUNGI</span></header>
      <section className={styles.content}>
        {view==="home"&&<><div className={styles.hero}><div><p className={styles.kicker}>BERHENTI · PERIKSA · LINDUNGI</p><h2>Pesan mencurigakan?<br/><em>Periksa sebelum percaya.</em></h2><p>Analisis tetap ephemeral meskipun kamu login. NusaGuard tidak menyimpan isi pesan atau membuat histori percakapan.</p><button onClick={()=>navigate("analyze")}>Mulai analisis →</button></div><div className={styles.shield}>N</div></div><div className={styles.cards}><button onClick={()=>navigate("analyze")}><span>01</span><h3>Analisis instan</h3><p>Klasifikasi enam kategori dan skor risiko.</p></button><button onClick={()=>navigate("analyze")}><span>02</span><h3>Penjelasan transparan</h3><p>Enam indikator N-SEAE yang mudah dipahami.</p></button><button onClick={()=>navigate("access")}><span>03</span><h3>Hak akses akun</h3><p>Lihat role dan izin fitur yang kamu miliki.</p></button></div></>}
        ...
        {view==="access"&&<section className={styles.access}><div><p className={styles.kicker}>ROLE & PERMISSION</p><h2>Akses akunmu</h2><p>Role menentukan fitur tambahan tanpa mengubah perlindungan dasar.</p></div><div><span className={styles.role}>{user.role}</span>{user.permissions.map(x=><p key={x}>✓ {x.replaceAll("_"," ")}</p>)}</div></section>}
      </section>
