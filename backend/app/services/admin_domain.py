...
class AdminDomain:
    ...
    def initialize(self) -> None:
        ...
        with self.engine.begin() as db:
            ...
            for slug, definition in ROLE_DEFINITIONS.items():
                db.execute(text("INSERT INTO roles(id,name,slug,description,is_system,created_at,updated_at) SELECT :id,:name,CAST(:slug AS VARCHAR(40)),:description,TRUE,:now,:now WHERE NOT EXISTS (SELECT 1 FROM roles WHERE slug=:slug)"), {"id": str(uuid.uuid4()), "name": definition["name"], "slug": slug, "description": f"Role sistem {definition['name']}", "now": now})
            for slug, (name, module) in PERMISSIONS.items():
                db.execute(text("INSERT INTO permissions(id,name,slug,module,description) SELECT :id,:name,CAST(:slug AS VARCHAR(80)),:module,:description WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE slug=:slug)"), {"id": str(uuid.uuid4()), "name": name, "slug": slug, "module": module, "description": name})
    ...
    def _seed_reference(self, db: Any, now: datetime) -> None:
        ...
        for phrase, indicator, weight in lexicons:
            db.execute(text("INSERT INTO nseae_lexicons(id,phrase,indicator,weight,match_type,example,description,is_active,created_at,updated_at) SELECT :id,CAST(:phrase AS VARCHAR(240)),CAST(:indicator AS VARCHAR(40)),:weight,'contains','',:description,TRUE,:now,:now WHERE NOT EXISTS (SELECT 1 FROM nseae_lexicons WHERE phrase=:phrase AND indicator=:indicator AND deleted_at IS NULL)"), {"id": str(uuid.uuid4()), "phrase": phrase, "indicator": indicator, "weight": weight, "description": "Leksikon awal NusaGuard", "now": now})
